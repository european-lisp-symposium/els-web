...
in the field of applied geometry he simulated real-world mechanical
engineering problems with previously unattained precision, speed,
efficiency, and robustness. This was implemented in Common Lisp.
If you have used a Hˆfler gear grinding machine in the last decade
there is a chance you profited from his work.

Max is a PhD candidate and worked as research and teaching assistant
at the KIT Computer Graphics Group in collaboration with the
Fraunhofer IOSB, Karlsruhe. He headed the software development of a
Berlin-based company providing services for the US insurance market
and served for three years as X12C TAS representative of the ANSI
Accredited Standards Committee X12 in the USA.

Currently, he works as a software developer at Datagraph, Berlin,
developing technology for the Dydra graph database service as well
as an independent scientific contractor and freelancer.

Since 2004 Max has been working as an artist and participated with
interactive installations in exhibitions in many European cities,
...
(define-programme-day @2022-03-21T00:00:00+01:00
  ...
  @09:15:00 (:title "Research Paper: Open Closures: Disclosing lambda's inner monomaniac object!"
             ...
             :description "While folklore teaches us that closures and objects are two sides of the 
same coin, they remain quite different in practice, most notably because 
closures are opaque, the only supported operation being to call them. 

In this article we discuss a few cases where we need functions to be less 
opaque, and propose to satisfy this need by extending our beloved $\lambda$ so 
as to expose as sorts of record fields some of the variables it captures. 
These \emph{open closures} are close relatives of CLOS's \emph{funcallable 
objects} as well as of the \emph{function objects} of traditional 
object-oriented languages like Java, except that they are functions made 
to behave like objects rather than the reverse. 
...
We show how practical usage of QueryFS looks like and what lies behind this.")
  @14:00:00 (:title "Research Paper: A CLOS protocol for lexical environments"
             ...
             :role (:talk)
             :description "The concept of an environment is mentioned in many places in the 
...
provided by the protocol, for implementation-specific purposes.")
  @14:30:00 (:title "Demo: IoT devices and embedded systems with uLisp"
             ...
             :role (:demo)
             :description "You've probably seen uLisp mentioned in the list of languages on the
...
OntoVPA projects.")
  ...
  @10:30:30 (:title "Creating a Common Lisp Implementation"
             ...
             :description "Being dissatisfied with the way current Common Lisp implementations
...
Julia is a recent programming language that targets some of the application areas of the widely popular Python language. However, the number of libraries available in Julia is still lower than what is provided in Python. We propose a solution that involves the development of a transpiler to generate human-readable Julia code from Python code, to speedup the translation of Python libraries to Julia.")
  @16:00:00 (:title "Research Paper: RacketLogger: Logging and Visualising Changes in DrRacket"
             ...
             :speakers ("Turgut Reis Kursun"
                        ...
                        "Quentin Stiévenart"
                        "Coen De Roover")
             :description "Developers frequently make code changes while programming, such as deleting a line of code and renaming or introducing a variable. 
